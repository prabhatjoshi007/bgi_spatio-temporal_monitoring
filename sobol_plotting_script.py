...
Updated to include intermediate plotting every N iterations.

@author: joshipra; USE THIS CODEE!!!!!!!!!!!!!
"""
...
# Discretise state inside the model
...
# Run simulations and periodically plot results
def run_sobol_simulations(base_folder, base_model, start_date, end_date, plot_interval=50):
    ...
    for i, lid_state in enumerate(sample):
        ...
        cso = abs((read_rpt_file(rpt_file).outfall_loading_summary['Total_Volume_10^6 ltr'].sum() - read_rpt_file(rpt_file).outfall_loading_summary['Total_Volume_10^6 ltr'].iloc[1]) - baseline_cso)
        Y.append(cso)

        if (i + 1) % plot_interval == 0 or (i + 1) == len(sample):
            print("Plotting figure...")
            plt.figure(figsize=(10, 4))
            plt.plot(Y, marker='o', linestyle='-', markersize=3, alpha=0.7)
            plt.axhline(y=0, color='r', linestyle='--')
            plt.title(f"CSO Volume over Simulations (up to run {i+1})")
            plt.xlabel("Simulation Number")
            plt.ylabel("Relative CSO Volume")
            plt.grid(True)
            plt.tight_layout()
            plt.savefig(f"progress_plot_run_{i+1}.png")
            plt.close()
...
Y, baseline = run_sobol_simulations(base_folder, base_model, start_date, end_date, plot_interval=50)
