...
# Generate samples
N = 500
sample = saltelli.sample(problem, N=N, calc_second_order=False)
        ...
        for i, line in enumerate(lines):
            if line.startswith('START_DATE'):
                ...
                if len(tokens) >= 9:
                    ...
                    tokens[1] = f"BR_CS{lid_state_row[lid_index]}"
...
def run_morris_simulations(base_folder, base_model, start_date, end_date):
    ...
    baseline_cso = read_rpt_file(baseline_rpt).outfall_loading_summary['Total_Volume_10^6 ltr'].sum() - read_rpt_file(baseline_rpt).outfall_loading_summary['Total_Volume_10^6 ltr'][1]

    Y = []
    for i, lid_state in enumerate(sample):
        #sim_folder = os.path.join(base_folder, f"Sim_{i+1}") 
        print(f"Running simulation no.: {i}")
        sim_folder = os.path.join(base_folder, "Sim_1")  
        ...
        # past_folder = os.path.join(base_folder, f"Sim_{i}")
        # if os.path.exists(past_folder):
        #     shutil.rmtree(past_folder)
        ...
        cso = read_rpt_file(rpt_file).outfall_loading_summary['Total_Volume_10^6 ltr'][1] - read_rpt_file(baseline_rpt).outfall_loading_summary['Total_Volume_10^6 ltr'][1]
        Y.append(cso)
...
Y, baseline = run_morris_simulations(base_folder, base_model, start_date, end_date)
...
# Annotate each point with the LID name
...
#plt.title("Sobol Sensitivity Analysis of BGIs", fontsize=14)
plt.grid(True)
plt.tight_layout()
plt.show()
